...
// Script para Corrigir Telefones usando dados do ASAAS
// Busca telefones corretos e atualiza a base local
...
async function fixPhoneNumbers() {
  console.log('📱 CORRIGINDO TELEFONES USANDO DADOS DO ASAAS');
  console.log('==============================================');
    ...
    for (let i = 0; i < problematicRegistrations.length; i++) {
      ...
      try {
        ...
        if (customerResponse.status === 200 && customerResponse.data.data.length > 0) {
          ...
          console.log(`   - Telefone no ASAAS: ${asaasPhone || 'Não informado'}`);

          if (asaasPhone && asaasPhone !== 'Não informado') {
            const normalizedPhone = normalizePhone(asaasPhone);
